...
public class MaterializePlugin : EditorWindow
{
    ...
    static void ProcessTextures(string[] texturePaths)
    {
        ...
        // e.g., Path name: "t_Grass_Diffuse_2k"
        string baseName = GetBaseName(texturePaths[0]);
        string outputPath = $"Assets/Materials/{baseName}/m_{baseName}.mat";
    ...
    static void ModifyTextureImportSettings(string texturePath, string textureName)
    {
        ...
        if (importer != null)
        {
            ...
            {
                // Preprocess to remove unecessary alpha channel from diffuse texture
                importer.alphaSource = TextureImporterAlphaSource.None; 
            }

            if (textureName.Contains("MAOS"))
            {
                // Preprocess to set MAOS texture to 
    ...
    static string GetBaseName(string path)
    {
        ...
        return parts.Length > 1 ? parts[1] : fileName; // Get only the second element if available
    }
}
